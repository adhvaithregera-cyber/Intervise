...
export async function POST(request: Request) {
  ...
  // ── Block duplicate submissions for the same answer slot ────────────────
  // Prevents a user from submitting the same answer_index twice, which would
  // create duplicate rows and skew the grade calculation.
  ...
  // ── Transcribe via AssemblyAI (API key never leaves server) ────────────
