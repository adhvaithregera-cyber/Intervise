    ...
    <div className="min-h-screen bg-[#EFE3CA] text-[#170C79]">
      ...
      <HeroHighlight containerClassName="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center px-4 sm:px-6">
        <div className="flex flex-col items-center text-center">
          <h1 className="mb-6 max-w-3xl text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
            <span style={{ color: '#56B6C6' }}>Your next interview is won{' '}</span>
            <span style={{ color: '#170C79' }}>before it starts.</span>
          </h1>

          <p className="mb-10 max-w-[520px] text-base leading-relaxed text-[#8ACBD0] sm:text-lg">
          ...
          <div className="flex flex-col items-center gap-3 sm:flex-row">
            {showTryFree && (
              <Link
                href="/signup"
                className="inline-flex w-full items-center justify-center rounded-xl bg-[#170C79] px-8 py-3 text-base font-semibold text-white shadow-md shadow-[#170C79]/20 hover:bg-[#0f0955] transition-colors sm:w-auto"
              >
                Practice for free
              </Link>
            )}
            <Link
              href="#how-it-works"
              className="inline-flex w-full items-center justify-center rounded-xl border-2 border-[#8ACBD0] bg-white px-8 py-3 text-base font-semibold text-[#170C79] hover:border-[#56B6C6] hover:text-[#56B6C6] transition-colors sm:w-auto"
            >
              See how it works →
            </Link>
          </div>
          ...
          {/* Trust line */}
          <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-[#8ACBD0]/60 bg-white px-4 py-1.5">
            <span className="text-xs text-[#8ACBD0]">No card needed</span>
            <span className="h-1 w-1 rounded-full bg-[#8ACBD0]" />
            <span className="text-xs text-[#8ACBD0]">2 free sessions</span>
            <span className="h-1 w-1 rounded-full bg-[#8ACBD0]" />
            <span className="text-xs text-[#8ACBD0]">Cancel anytime</span>
          </div>
        </div>
      </HeroHighlight>

      {/* Category strip — bleeds from hero into How It Works */}
      <div className="relative z-10 -mt-6 border-y border-[#8ACBD0]/40">
        <CategoryStrip />
      </div>
      ...
      <section id="how-it-works" className="py-20 sm:py-24" style={{ background: 'linear-gradient(to bottom, #EFE3CA 0%, #EFE3CA 80%, #e8dbbf 100%)' }}>
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          ...
          <h2 className="mb-4 text-3xl font-bold text-[#170C79] sm:text-4xl">How it works</h2>
          <p className="mb-12 max-w-xl text-[#8ACBD0]">
          ...
          <div className="space-y-5">
            {steps.map((step, i) => (
              <div
                key={step.number}
                className="group grid grid-cols-[auto_1fr] gap-5 rounded-2xl border border-[#8ACBD0] bg-white p-6 shadow-sm hover:border-[#56B6C6] hover:shadow-md transition-all sm:gap-8 sm:p-8"
              >
                ...
                <div className="flex flex-col items-center">
                  ...
                  {i < steps.length - 1 && (
                    <div
                      ...
                      style={{
                        background: 'linear-gradient(to bottom, #8ACBD0, transparent)',
                        minHeight: '1.5rem',
                      }}
                    />
                  )}
                ...
                <div>
                  <div className="mb-3 flex flex-wrap items-center gap-2 sm:gap-3">
                    <h3 className="text-lg font-bold text-[#170C79] sm:text-xl">{step.title}</h3>
                    <span className="rounded-full border border-[#8ACBD0] bg-[#EFE3CA] px-3 py-0.5 text-xs font-semibold text-[#8ACBD0]">
                      {step.tag}
                    </span>
                  </div>
                  <p className="mb-5 text-sm leading-relaxed text-[#8ACBD0]">{step.description}</p>
                  <ul className="mb-4 space-y-2">
                    {step.details.map((d) => (
                      <li key={d} className="flex items-start gap-2.5 text-sm text-[#170C79]">
                        <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#56B6C6]" />
                        {d}
                      </li>
                    ))}
                  </ul>
                  {step.formats.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {step.formats.map((f) => (
                        <span
                          key={f}
                          className="rounded-xl border border-[#56B6C6]/40 bg-[#56B6C6]/10 px-3 py-1 text-xs font-semibold text-[#170C79]"
                        >
                          {f}
                        </span>
                      ))}
                      <span className="rounded-xl border border-[#8ACBD0] bg-[#EFE3CA] px-3 py-1 text-xs font-semibold text-[#8ACBD0]">
                        +3 more
                      </span>
                    </div>
                  )}
      ...
      <section className="bg-[#EFE3CA] py-20 sm:py-24">
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          ...
          <h2 className="mb-4 text-3xl font-bold text-[#170C79] sm:text-4xl">
            Same candidate. Same question.
          </h2>
          <p className="mb-10 max-w-xl text-[#8ACBD0]">
      ...
      {/* Final CTA banner */}
      <section className="bg-[#EFE3CA]/40 px-4 py-20 text-center sm:px-6 sm:py-24">
        <h2 className="mb-4 text-3xl font-bold text-[#170C79] sm:text-4xl">
          Your interview is closer than you think.
        </h2>
        <p className="mb-8 text-[#8ACBD0]">
          Start practising today — free, no card required.
        </p>
        <Link
          href="/signup"
          className="inline-flex items-center rounded-xl bg-[#170C79] px-10 py-4 text-base font-semibold text-white shadow-lg shadow-[#170C79]/20 hover:bg-[#0f0955] transition-colors"
        >
          Start FREE
        </Link>
      </section>
      ...
      <footer className="border-t border-[#8ACBD0] bg-[#EFE3CA]">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-3 px-4 py-6 text-center text-sm text-[#8ACBD0] sm:flex-row sm:justify-between sm:px-6 sm:text-left">
          ...
          <div className="flex items-center gap-4">
            <Link href="/privacy" className="hover:text-[#170C79] transition-colors">
              Privacy Policy
            </Link>
            <span className="text-[#8ACBD0]">·</span>
            <Link href="/terms" className="hover:text-[#170C79] transition-colors">
              Terms of Service
            </Link>
          </div>
        </div>
